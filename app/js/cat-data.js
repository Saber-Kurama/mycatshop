...
let IMG_URL = 'http://lorempixel.com/600/400/cats/';

//missing 6 is intentional, IIRC it's an ugly cat :)
let catImages = [1, 2, 3, 4, 5, 7, 8, 9, 10].map(function(n) { return util.format('images/%s.jpg', n); });
let catNames = [
  'Skittles',
  'Petey',
  'Patches',
...
function getCats(num, lowPrice, highPrice) {
  let images = catImages.map(function(img, i) {
    return {img: img, id: i};
  });

  let names = catNames.map(function(name, i) {
    return {name: name, id: i};
  });

  let quotes = catQuotes.map(function(quote, i) {
  ...
  for (let i = 0; i < num; ++i) {
    ...
    let img = images.splice(imageIndex, 1)[0];
    let name = names.splice(nameIndex, 1)[0];
    let quote = quotes.splice(quoteIndex, 1)[0];
